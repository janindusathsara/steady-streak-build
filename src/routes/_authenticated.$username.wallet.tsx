...
export const Route = createFileRoute("/_authenticated/$username/wallet")({
  component: WalletRoute,
});

function WalletRoute() {
  const { username } = useParams({ from: "/_authenticated/$username/wallet" });
  const { loading, profile } = useCurrentUser();
  const navigate = useNavigate();
  useEffect(() => {
    if (!loading && profile && profile.username.toLowerCase() !== username.toLowerCase()) {
      navigate({ to: "/$username/wallet", params: { username: profile.username }, replace: true });
    }
  ...
  return <HomeownerDashboard initialTab="wallet" />;
}
