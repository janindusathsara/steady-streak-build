...
export const Route = createFileRoute("/_authenticated/$username/active-bookings")({
  component: ActiveBookingsRoute,
});

function ActiveBookingsRoute() {
  const { username } = useParams({ from: "/_authenticated/$username/active-bookings" });
  const { loading, profile } = useCurrentUser();
  const navigate = useNavigate();
  useEffect(() => {
    if (!loading && profile && profile.username.toLowerCase() !== username.toLowerCase()) {
      navigate({ to: "/$username/active-bookings", params: { username: profile.username }, replace: true });
    }
  ...
  return <HomeownerDashboard initialTab="active" />;
}
