...
import { AdminMonthlyOverviewPage } from "@/pages/admin/AdminMonthlyOverviewPage";
import { useCurrentUser } from "@/hooks/use-current-user";
import { userDashboardPath } from "@/lib/role";

export const Route = createFileRoute("/_authenticated/$username/overview")({
  component: OverviewRoute,
});

function OverviewRoute() {
  const { username } = useParams({ from: "/_authenticated/$username/overview" });
  const { loading, profile } = useCurrentUser();
  const navigate = useNavigate();
  useEffect(() => {
    if (!loading && profile && profile.username.toLowerCase() !== username.toLowerCase()) {
      navigate({ to: "/$username/overview", params: { username: profile.username }, replace: true });
    }
  ...
  return <AdminMonthlyOverviewPage />;
}
